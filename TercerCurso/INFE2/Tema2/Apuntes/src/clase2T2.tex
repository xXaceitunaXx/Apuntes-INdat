%Parece ser que esto es le tema 4
\section{Test de Bondad de Ajuste (TBA)}

Hasta ahora los problemas planteados parten de unos datos obtenidos
en un experimento aleatorio del que se conoce el mecanismo con el que se genera (Familia de la distribución)

El Test de Bondad de Ajuste va a ser unn test para comprobar si una familia de distribuciones, representa correctamente el mecanismo con el que se generaron los datos.

%Se pone a hacer un gráfico de una funcion de distribución empírica

El plantaeamiento del tets es que tenemos $X_1,\dots,X_n$ con funcion de distribución F, si $F_0$ es una función de distribución conocida, por ejemplo, una Poisson con $\lambda=3$.

El problema se reduce a:
$$H_0: F=F:_0 \quad H_1: F \neq F_0$$
En este caso $F_0$ está completamente especificada, entonces es una hipótesis simple.

La hipótesis nula es compuesta si $F_0$ depende de parámetros desconocidos, por ejemplo, una P($\lambda$)

Empezamos con el caso de $H_0$ simple. Existen dos tipos de TBA si $F_0$ es una función de  distribución conocida.
\begin{enumerate}
    \item $F_0$ discreta: Se comparan las frecuencias observadas con esperadas bajo $H_0$. Test $\chi^2$. También se puede hacer si $F_0$ es continua agrupando, pero hay tests más potentes para esos casos.
    \item $F_0$ continua: Comparamos la funcion de distribución empírica con la teórica.
    
    Ejemplo: Kolmogorov-Smirnov
    Meter el gráfico.
    
    
\end{enumerate}

Test $\chi^2$ de bondad de ajuste.
...
\[
H_0:\quad p_1=\frac{1}{3} \quad p_2=\frac{1}{3} p_3=\frac{1}{3}
\]
...
\]
 $\frac{(f_j-e_j)^2}{e_j}$ es lo que se conoce como distancia $\chi^2$ entre $f_j$ y $e_j$.
...
\[
T = 2 \left[\log L(\hat{p}, x) - \log L(p_0, x)\right] = (*)
\]
...
\[
(*) = 2 \left[ \sum_{j=1}^{k} f_j \log \hat{p_j} - \sum_{j=1}^{k} f_j \log p_j^0 \right] 
...
Esta fórmula no está acabada.

Finalmente, esto converge a 0, ya que, por el teorema de la Ley de los Grandes Números (LGN), \( \frac{F_j}{n} \) es un estimador consistente de \( p_j \):

\[
\lim_{n \to \infty} P \left( \left| \frac{F_j}{n} - p_j \right| > \varepsilon \right) = 0
\]

Por lo tanto, \( (**) \) se convierte en:

\[
(**) = -2 \sum_{j=1}^{k} \left( n \cdot p_j^0 - f_j \right) + \frac{\sum_{j=1}^{k} (f_j - n \cdot p_j^0)^2}{f_j}
\]
