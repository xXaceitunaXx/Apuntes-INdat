...
Es aquí donde surje el concepto de versión. Para continuar adaptandose y expandiendose a las necesidades, el
...
\textbf{buen} Software. Los atributos de un buen producto (según la asignatura de FIS) Software son:
...
\begin{enumerate}
    ...
    \begin{enfasis}
        ...
        eficientemente, debido a necesidades evolutivas, correctivas, adaptativas o perfectivas.
    \end{enfasis}
...
\noindent La Ingeniería de Software se define como
...
\noindent Desde distintos puntos de vista:
\begin{enumerate}
...
\textit{Esto está textualmente copiado de las transparencias de clase, por que lo vi importante y tampoco sabía 
que más añadir, pero creo que deja más que claro lo que es, ¿no? De momento somo hemos dicho lo que es, falta 
...
problema para tener la capacidad de explicarlo con máximo detalle en un ``lenguaje'' que permita interpretarlo y
resolverlo (esta interpretación es mía, no algo que haya que aprenderse de pe a pa).

También se trata de una disciplina de resolución de problemas, pues como cualquier ingeniería, suerge sobre la 
necesidad de resolver problemas mediante la adquisición de conocimiento y la toma razonada de decisiones 
(y justificable). En general, todo lo que no podamos explicar, o que no este lo suficientemente documentado, no 
está bien hecho.
...
El \textbf{modelo} es la representación abstracta y semantica\footnote{El modelo semántico es un modelo conceptual
 de datos en el que se incluye información semántica. Esto significa que el modelo describe el significado de 
sus instancias. \href{https://es.wikipedia.org/wiki/Modelo_sem\%C3\%A1ntico_de_datos}{Fuente: Wikipedia}} de un 
sistema para analizarlo y comprenderlo, lo que nos permite responder preguntas sobre el mismo (es la forma de 
abordar la complejidad de un sistema).

Un ingeniero de Software debe comprender el entorno en que ha de operar el sistema, esto se conoce como 
\textbf{dominio del problema}, pero tambien necesita comprender el sistema a construir para poder evaluar las 
distintas soluciones posibles. Esto último es el \textbf{dominio de la solución}. Los modelos de dominio del
problema se construyen sobre aquellos elementos del mundo real que son relevantes para el problema en 
consideración, es una comprensión profunda del entorno en donde ha de operar el sistema. Los modelos de dominio
de la solución recogen todo lo relacionado a la comprensión y evaluación de los sistemas a construir.

\subsection{Actividad de resolución de problemas}

La actividad de resolución de problemas es un proceso esencial para la Ingeniería de Software, caracterizada por
buscar soluciones adecuadas a problemas complejos. Existen multiples condicionantes que hacen de esta una ardua
tarea, como la evaluación empírica de soluciones (toda conclusión se debe extraer de pruebas empíricas
\footnote{Lo empírico consiste en aquello que se puede experimentar o comprobar a traves de los sentidos, 
un encuentro directo con la realidad}), el manejo de recursos limitados y la necesidad de trabajar con 
conocimiento incompleto (no nos lo van a dar todo hecho por desgracia). Para abordar un problema, se sigue un 
método estructurado que consta de las siguientes fases:

\begin{enumerate}
    \item \textbf{Formulación del problema}. Debemos definir con \textit{claridad} lo que se desea resolver.
    \item \textbf{Análisis del problema}. Comprende el entendimiento de las causas, las restricciones y las
    posibles implicaciones.
    \item \textbf{Búsqueda de soluciones}. Se exploran todas las alternativas posibles.
    \item \textbf{Eleccion de la solución más adecuada}. Conocidas las alternativas, evaluamos las soluciones y
    escogemos la más eficaz.
    \item \textbf{Especificación de la solución}. Finalmente, se detalla como se implementa la solución escogida.
\end{enumerate}

Para poder llevar a cabo este proceso, es imprescindible contar con la información adecuada.

\subsection{Actividad de conocimiento y toma de decisiones}

Otro de los procesos esenciales en ingeniería de sotware es la actividad de conocimiento y toma de decisiones, ya que influye en todas las fases del desarrollo de software,
 desde la planificación inicial hasta el mantenimiento y evolución del sistema. Su principal utilidad es la toma de decisiones informadas que impacten positivamente en el proyecto.

La actividad de adquisición de conocimiento, abarca todo el proceso de obtención tanto de conocimiento técnico (lenguajes de programación, arquitecturas de software,$\dots$) como de conocimiento del dominio (area en que se está desarrollando el software). Hay que tener en cuenta que la adquisición de conocimiento es un proceso no lineal y en el que si no se realiza adecuadamente puede ser necesario volver a empezar.

La actividad de toma de decisiones consiste en seleccionar la mejor opción entre diversas alternativas, utilizando el conocimiento adquirido y considerando las restricciones del proyecto, para que así puedan ser justificadas.

\subsection{Disciplinas integradas en la ingeniería de software (SWEBOK)}

El SWEBOK (Software Engineering Body of Knowledge) organiza el conocimiento en diferentes disciplinas integradas. Estas se agrupan en discliplinas técincas y de gestión.

\textbf{Disciplinas técnicas: } Se enfocan en la creación, desarrollo y mantenimiento del software desde el punto de vista técnico. Estas disciplinas son:  
\begin{itemize}
    \item \textit{Requisitos de Software: }Es el proceso de definir, documentar y gestionar las necesidades y expectativas de los usuarios.
    \item \textit{Diseño de Software: }Proceso para definir la arquitectura, componentes, interfaces y características de un sistema o sus componentes.
    \item \textit{Construcción de Software: }Codificación e implementación del software siguiendo el diseño establecido.
    \item \textit{Prueba de Software: }Actividad que garantiza que el software cumple con los requisitos especificados y funciona correctamente.
    \item \textit{mantenimiento de Software: }Proceso de modificación del software después de su entrega para corregir defectos, mejorar el rendimiento u otros atributos.
\end{itemize}

\textbf{Disciplinas de gestión: }Estas disciplinas están relacionadas con la organización, planificación y control del proceso de desarrollo de software.
\begin{itemize}
    \item \textit{Gestión de la Configuración de Software: }Proceso que controla y gestiona los cambios en el software.
    \item \textit{Gestión de la Ingeniería del Software: }Planificación, coordinación, medición, monitoreo, control y reporte de proyectos de software.
    \item \textit{Proceso de la Ingeniería de Software: }Conjunto de actividades, métodos, prácticas y transformaciones que los ingenieros de software utilizan para desarrollar y mantener el software.
    \item \textit{Herramientas y Métodos de la Ingeniería de Software: }Uso de herramientas y técnicas que apoyan el desarrollo y la gestión del software.
    \item \textit{Calidad del Software: }Aseguramiento de que el software cumple con los estándares, requisitos y expectativas del cliente.
\end{itemize}
